...
def get_price_detail(div):
    try:
        array=div.find("div", id="table-price-1").find_all("ul")
        #mỗi ul lấy từng li luu vào mảng
        array = [[li.text.replace("\r", "").replace("\t"," ").replace("\n","").strip().replace("\xa0"," ") for li in ul.find_all("li")] for ul in array]
...
def get_policy(div):
    try:
        ...
        policy_register=tour_rules_tag.find(lambda tag: tag.name=='h3' and tag.text.strip()=='QUY TRÌNH ĐĂNG KÝ TOUR').findNextSibling().text.strip()
        ...
        policy_note=div.find("div", id="tour-rule-2").find(lambda tag: tag.name=='h3' and tag.text.strip()=='NHỮNG LƯU Ý KHÁC').findNextSibling().text.strip()
...
url_P="https://www.vietnambooking.com/du-lich/tour-nha-trang-dao-yen-3n3d.html"
try_pull(url_P)
